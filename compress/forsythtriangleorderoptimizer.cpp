...
namespace Forsyth
{
    ...
    void OptimizeFaces(const uint32* indexList, uint indexCount, uint vertexCount, uint32* newIndexList, uint16 lruCacheSize);
...
//        }
    ...
    void OptimizeFaces(const IndexType* indexList, uint indexCount, uint vertexCount, IndexType* newIndexList, uint16 lruCacheSize)
    {
        ...
        for (uint i = 0; i < indexCount; i += 3)
        {
            if (bestScore < 0.f)
            {
                ...
                for (uint j = 0; j < indexCount; j += 3)
                {
                    if (processedFaceList[j] == 0)
                    {
                        ...
                        for (uint k=0; k<3; ++k)
                        {
                            IndexType index = indexList[face+k];
                            OptimizeVertexData& vertexData = vertexDataList[index];
            ...
            for (uint c1 = 0; c1 < entriesInCache1; ++c1)
            {
                ...
                for (uint j=0; j<vertexData.activeFaceListSize; ++j)
                {
                    ...
                    for (uint v=0; v<3; v++)
                    {
                        IndexType faceIndex = indexList[face+v];
    ...
    void OptimizeFaces(const uint32* indexList, uint indexCount, uint vertexCount, uint32* newIndexList, uint16 lruCacheSize)
    {
      return OptimizeFaces<uint32>(indexList, indexCount, vertexCount, newIndexList, lruCacheSize);
    }

    void OptimizeFaces(const uint16* indexList, uint indexCount, uint vertexCount, uint16* newIndexList, uint16 lruCacheSize)
    {
      return OptimizeFaces<uint16>(indexList, indexCount, vertexCount, newIndexList, lruCacheSize);
    }
