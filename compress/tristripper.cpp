...
namespace triangle_stripper
{
  ...
  namespace detail
  {
    ...
    namespace
    {
      ...
      void LinkNeighbours(
          graph_array<triangle>& Triangles, const edge_map& EdgeMap, const tri_edge Edge);
    }
  ...
  inline tri_stripper::const_link_iterator tri_stripper::BackLinkToNeighbour(
      const_tri_iterator Node, bool ClockWise, triangle_order& Order)
  {
  ...
  inline void tri_stripper::AddTriangle(
      const triangle& Tri, const triangle_order Order, const bool NotSimulation)
  {
    switch (Order)
    {
      case ABC:
  ...
  inline bool tri_stripper::Cache() const { return (m_Cache.size() != 0); }

  inline size_t tri_stripper::CacheSize() const { return m_Cache.size(); }
